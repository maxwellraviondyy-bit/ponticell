...
export default function App() {
  ...
  const [testiItems, setTestiItems] = useState([]); // [{preview, url, keterangan, uploading}]
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (error || !data) {
      ...
      if (newAttempts >= MAX_LOGIN_ATTEMPTS) {
        ...
        setLoginAttempts(0);
        try { localStorage.setItem("ponticell_login_locked_until", String(lockedUntil)); localStorage.setItem("ponticell_login_attempts", "0"); } catch {}
        setLoginError(`Login diblokir 60 menit karena ${MAX_LOGIN_ATTEMPTS}x percobaan gagal.`);
      } else {
        setLoginAttempts(newAttempts);
        try { localStorage.setItem("ponticell_login_attempts", String(newAttempts)); } catch {}
    ...
    try { localStorage.removeItem("ponticell_login_attempts"); localStorage.removeItem("ponticell_login_locked_until"); } catch {}
  ...
  const readyInventory = inventory.filter((item) => {
    ...
    const matchRam = selectedRam === "Semua" || (item.ram || "-") === selectedRam;
  ...
  const ramOptions = ["Semua", ...Array.from(new Set(
    inventory
      .filter(i => i.type === productType && Object.values(i.stocks || {}).reduce((s, v) => s + v, 0) > 0 && i.ram && i.ram !== "-")
      .map(i => i.ram)
      ...
      <div style={c.nav}>
        {[
          ...
          <button key={tab} style={c.navBtn(activeTab === tab)} onClick={() => {
            setActiveTab(tab);
            if (tab === "hp") { setProductType("hp"); setSelectedRam("Semua"); }
            if (tab === "tablet") { setProductType("tablet"); setSelectedRam("Semua"); }
        ...
        {(activeTab === "hp" || activeTab === "tablet") && (
          <>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: "#94A3B8", marginRight: 2 }}>RAM:</span>
              {ramOptions.map(ram => (
                <button key={ram} onClick={() => setSelectedRam(ram)}
                  style={{ padding: "5px 12px", borderRadius: 20, border: selectedRam === ram ? "2px solid #FF6B35" : "2px solid #E2E8F0", background: selectedRam === ram ? "#FFF1EB" : "#fff", color: selectedRam === ram ? "#FF6B35" : "#64748B", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Plus Jakarta Sans', sans-serif", transition: "all 0.15s" }}>
                  {ram === "Semua" ? "Semua" : `${ram} GB`}
                </button>
              ))}
            </div>
